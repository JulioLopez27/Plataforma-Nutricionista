...
export class Nutricionista {
  ...
  agregarConsultante(consultante) {
    ...
    if (consultante instanceof Consultante) {
      ...
      throw new Error(
        "El parámetro debe ser una instancia de la clase Consultante"
      );
    }
  }
  ...
  static async getProfileData(ctx) {
    ...
    try {
      ...
      if (!getProfileCountry) {
        ctx.status = HTTP_STATUS_NOT_FOUND;
        throw new Error(
          "No se encontró el nutricionista o no tiene país asociado."
        );
      }

      const getProfileSpecialty =
        await prisma.nutricionista_especialidad.findFirst({
          where: {
            id_nutricionista: userId,
          },
          select: {
      ...
      if (!getProfileSpecialty) {
        ctx.status = HTTP_STATUS_NOT_FOUND;
        throw new Error(
          "No se encontró el nutricionista o no tiene especialidad asociada."
        );
      }

      const { nombre, apellido, telefono, email, anos_experiencia } =
        getProfileData;
  ...
  static async signup(ctx) {
    try {
      ...
      const idChef =
        parseInt(ctx.request.body.id_chefDigitales) ||
        Nutricionista.set_default_idChefDigitales;
  ...
  static async preRegistro(user_data, id_pais, ciudad, id_especialidad) {
    try {
      await fetch(
        "https://cheffdigital.com/cheffadmin/api/api3.php/registroUsuarioNutricionista",
        {
          method: "post",
          headers: {
  ...
  static async updateProfile(ctx) {
    ...
    try {
      const id_pais = await stringToInt(ctx.request.body.pais);
      const id_especialidad = await stringToInt(ctx.request.body.especialidad);
      const anos_experiencia = await positiveValue(
        ctx.request.body.anos_experiencia
      );
      ...
      await updateRecord(
        prisma.nutricionista_pais,
        userId,
        profile_countrie_data
      );
